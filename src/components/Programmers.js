...
export default function Programmers() {
  ...
  const [developer, setDeveloper] = useState(listOfAwesome);
  ...
  const getNameOfFeatured = () => {
    ...
    return developer[devID - 1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: Programmers ? 'gold' : 'royalblue' // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          developer.map((dev) =>
            <div className='programmer' key={dev.id}>
              {dev.name}{" "}
              <button onClick={() => { setDevID(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
